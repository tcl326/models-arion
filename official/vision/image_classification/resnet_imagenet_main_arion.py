...
import os, sys 
import time
sys.path.append('/tmp/arions/scalability/models-master')
...
def run(flags_obj):
  ...
  if flags_obj.arion_strategy == 'PS':
    arion = Arion(resource_spec_file, PS())
    #, runner_config_file=config_file)
  elif flags_obj.arion_strategy == 'PSLoadBalancing':
    arion = Arion(resource_spec_file, PSLoadBalancing())
    #, runner_config_file=config_file)  
  elif flags_obj.arion_strategy == 'PartitionedPS':
    arion = Arion(resource_spec_file, PartitionedPS())
    #, runner_config_file=config_file)    
  elif flags_obj.arion_strategy == 'AllReduce':
    arion = Arion(resource_spec_file, AllReduce(128))
    #, runner_config_file=config_file)    
  elif flags_obj.arion_strategy == 'Parallax':
    arion = Arion(resource_spec_file, Parallax())
    #, runner_config_file=config_file)
  elif flags_obj.arion_strategy == 'SFB':
    arion = Arion(resource_spec_file, SFB())
  else:
  ...
  #     tpu_address=flags_obj.tpu)python3 /tmp/arion/scalapython3 /tmp/arion/scalability/models-master/official/vision/image_classification/resnet_imagenet_main_arion.py --data_dir=/tmp/arion/scalability/train1 --train_epochs=10 --cnn_model=densenet121 --is_arion=Truebility/models-master/official/vision/image_classification/resnet_imagenet_main_arion.py --data_dir=/tmp/arion/scalability/train1 --train_epochs=10 --cnn_model=densenet121 --is_arion=True
...
def main(_):
  ...
  if flags.FLAGS.is_arion:
    logname = 'log_strategy_{}_model_{}_node_{}_gpu_{}_patch_{}'.format(flags.FLAGS.arion_strategy,
      flags.FLAGS.cnn_model, node_num, gpu_num, flags.FLAGS.arion_patch_tf)
  else:
