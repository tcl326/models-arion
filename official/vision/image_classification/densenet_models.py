...
def DenseNet(blocks,
    ...
    x = transition_block(x, 0.5, name='pool2', training=training)
    x = dense_block(x, blocks[1], name='conv3', training=training)
    if len(blocks) > 2:
        x = transition_block(x, 0.5, name='pool3', training=training)
        x = dense_block(x, blocks[2], name='conv4', training=training)
        x = transition_block(x, 0.5, name='pool4', training=training)
